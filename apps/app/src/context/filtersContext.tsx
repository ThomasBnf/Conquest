...
import { Category, GroupFilter, NodeListRecords } from "@/schemas/node.schema";
...
export const FiltersProvider = ({ node, children }: Props) => {
  ...
  const onDeleteGroup = (groupFilter: GroupFilter) => {
    ...
    const updatedGroupFilters = groupFilters.filter((group) => group.id !== groupFilter.id);
  ...
  const onAddFilter = ({ groupFilter, filter }: FilterProps) => {
    ...
    const updatedGroupFilters = groupFilters.map((group) =>
      group.id === groupFilter.id ? { ...group, filters: [...group.filters, newFilter] } : group,
    );
