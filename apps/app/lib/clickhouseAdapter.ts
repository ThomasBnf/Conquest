...
export function parseDateTimeFields<T extends object>(row?: T) {
  ...
  if (
    row &&
    "emailVerified" in row &&
    row.emailVerified &&
    typeof row.emailVerified === "string"
  ) {
    parsedRow.emailVerified = parseUTCDate(row.emailVerified);
    if (Number.isNaN(parsedRow.emailVerified.getTime())) {
      console.warn(
        `Invalid date format for emailVerified: ${row.emailVerified}`,
      );
      parsedRow.emailVerified = undefined;
    } else {
      console.log(
        `Parsed emailVerified as Date object: ${parsedRow.emailVerified.toISOString()}`,
      );
    }
  }
  if (
    ...
    if (Number.isNaN(parsedRow.expires.getTime())) {
      console.warn(`Invalid date format for expires: ${row.expires}`);
      parsedRow.expires = undefined;
    } else {
      console.log(
        `Parsed expires as Date object: ${parsedRow.expires.toISOString()}`,
      );
    }
  }
  ...
  if (
    ...
    parsedRow.expires_at = Number.parseInt(row.expires_at);
    if (Number.isNaN(parsedRow.expires_at)) {
...
function parseUTCDate(dateString: string): Date {
  ...
  if (!match) {
    throw new Error(
      `Invalid date format: ${dateString}. Expected format: YYYY-MM-DD HH:MM:SS.fff`,
    );
  }
  ...
  console.log(
    `Parsed date string "${dateString}" into Date: ${date.toISOString()}`,
  );
  return date;
}
...
export default function ClickhouseAdapter(client: ClickHouseClient): Adapter {
  ...
  const testConnection = async () => {
    try {
      ...
      throw new Error(
        "Failed to connect to ClickHouse. Please check your client configuration.",
      );
    }
  };
  ...
  return {
    ...
    async useVerificationToken({
      ...
      try {
        ...
        if (rows.length === 0) {
          console.warn(
            `No verification token found for identifier: ${identifier} and token: ${token}`,
          );
          return null;
        }

        const deleteQuery =
          "ALTER TABLE verificationToken DELETE WHERE identifier = { identifier: String } AND token = { token: String }";
        ...
        console.log(
          `Verification token ${token} used and deleted successfully.`,
        );
    ...
    async createUser(user: Partial<AdapterUser>): Promise<AdapterUser> {
      ...
      const newUser = {
        ...
        emailVerified: emailVerified
          ? formatDate(new Date(emailVerified))
          : null,
        image: image || "",
      };
      ...
      try {
        ...
        const userToInsert = {
          ...
          emailVerified: updatedUser.emailVerified
            ? formatDate(updatedUser.emailVerified)
            : null,
        };
    ...
    async deleteUser(userId: string): Promise<void> {
      try {
        ...
        await client.exec({
          query:
            "ALTER TABLE sessions DELETE WHERE userId = { userId: String }",
          query_params: { userId },
        });
        ...
        await client.exec({
          query:
            "ALTER TABLE accounts DELETE WHERE userId = { userId: String }",
          query_params: { userId },
        });
    ...
    async linkAccount(account: AdapterAccount): Promise<AdapterAccount> {
      if (!account.provider || !account.providerAccountId || !account.userId) {
        throw new Error(
          "Provider, providerAccountId, and userId are required for account linking",
        );
      }
    ...
    async unlinkAccount({ providerAccountId, provider }): Promise<void> {
      if (!providerAccountId || !provider) {
        throw new Error(
          "Provider and providerAccountId are required for unlinking account",
        );
      }
    ...
    async createSession({
      ...
      try {
        ...
        if (rows.length === 0) {
          throw new Error(
            "Failed to create session: Session not found after insertion",
          );
        }
    ...
    async getSessionAndUser(sessionToken: string | undefined): Promise<{
      ...
      try {
        const sessionRes = await client.query({
          query:
            "SELECT * FROM sessions where sessionToken = { sessionToken: String }",
        ...
        if (userRows.length === 0) {
          console.error(
            `Session references non-existent user: ${session.userId}`,
          );
          return null;
        }
    ...
    async updateSession(
      ...
      await client.exec({
        query:
          "ALTER TABLE sessions DELETE WHERE sessionToken = { sessionToken: String }",
        query_params: { sessionToken },
      });
        ...
        values: [
          {
            ...
            expires: newSession.expires
              ? formatDate(new Date(newSession.expires))
              : null,
      ...
      const selectQuery =
        "SELECT * from sessions where sessionToken = { sessionToken: String } AND expires = { expires: String }";
