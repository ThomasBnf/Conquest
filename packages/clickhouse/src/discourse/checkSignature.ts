...
export const checkSignature = async ({ request }: Props) => {
  ...
  const result = await client.query({
    query: `
      SELECT *
      FROM integrations
      WHERE JSONExtractString(CAST(details AS String), 'source') = 'Discourse'
      AND JSONExtractString(CAST(details AS String), 'community_url') = ${community_url}
    `,
  });

  const { data } = await result.json();
  return DiscourseIntegrationSchema.parse(data);
};
