...
import { SlackProfileSchema } from "@conquest/zod/schemas/profile.schema";
...
export const seed = schemaTask({
  ...
  run: async ({ user }) => {
    ...
    for (const workspace of workspaces) {
      ...
      if (!slack) {
        logger.info(`No slack integration found for workspace ${workspaceId}`);
        continue;
      }

      const slackIntegration = SlackIntegrationSchema.parse(slack);
      ...
      const result = await client.query({
        query: `
          SELECT *
          FROM profile
          WHERE JSONExtractString(CAST(attributes, 'String'), 'source') = 'Slack'
            AND NOT has(JSONExtractKeys(CAST(attributes, 'String')), 'displayName')
      ...
      for (const profile of profiles) {
        ...
        const { display_name } = user?.profile ?? {};
