    ...
    while (hasNextNode) {
      ...
      switch (type) {
        case "list-members": {
          ...
          const parsedMembers = z.array(MemberSchema).parse([]);
...
export const listMembers = async (workspace_id: string) => {
  ...
  return MemberSchema.array().parse(
    members.map((member) => ({
      ...member,
...
export const filterMembers = (members: Member[], filters: Filter[]) => {
...
export const createSelectFilter = (filter: FilterSelect) => {
  ...
  return {
    execute: ({ member }: { member: Member }) => {
      ...
      //     case "language":
      //       return member.locale?.split("_")[0];
      //     case "tags":
      //       return member.tags;
      //     case "source":
